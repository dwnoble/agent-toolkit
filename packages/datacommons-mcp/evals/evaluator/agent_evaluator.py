...
class AgentEvaluator:
  ...
  def _create_results_dataframe(
      eval_metric_results: dict[str, list[_EvalMetricResultWithInvocation]], # Using Any for _EvalMetricResultWithInvocation
      num_runs: int,
  ) -> pd.DataFrame:
    """
    ...
    for metric_name, eval_metric_results_with_invocations in eval_metric_results.items():
      ...
      for invocation_idx, per_invocation_result in enumerate(eval_metric_results_with_invocations, 1):
